...
namespace SunFFB
{
    class FFBForceCalculator
    {
        ...
        void condition_force_calculator(const EffectBlock& effectBlock, const int32_t metrics[NUM_AXIS], float forces[NUM_AXIS]) const;

        float get_envelope(const SetEnvelopeReportData& envelopeData, uint32_t elapsedTime, uint16_t duration) const;
        float apply_condition(const SetConditionReportData& conditionData, int32_t metric) const;
