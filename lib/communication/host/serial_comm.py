        ...
        while self.running:
            try:
                while True:
                    ...
                    if len(buffer) == 0:
                        if byte == b'\x55': buffer.extend(byte)
                        continue

                    if len(buffer) == 1:
                        self.packet_length = int.from_bytes(byte, 'little')
                        if self.packet_length < 2 or self.packet_length > packet.MAX_DATA_LENGTH:
                            buffer.clear()
                            continue
                        buffer.extend(byte)
                        continue
                    ...
                    buffer.extend(byte)
                    if len(buffer) == (self.packet_length + 2):
                        result = packet.decode_packet(buffer[2:])
                        if result:
            ...
            if text:
                try:
                    ...
                    if text[0] > -1:
                        ...
                        codes = packet.build_packet(text[0], struct.pack('<fBB', text[1], text[2], text[3]))
