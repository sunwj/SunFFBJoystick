        ...
        while self.running:
            try:
                while True:
                    ...
                    if (len(buffer) >= 1) or (len(buffer) == 0 and byte == b'\x55'):
                        buffer.extend(byte)
                        if len(buffer) == packetSize:
                            if buffer[-1] == packet.compute_checksum(buffer[1:-1]):
                                a, b, c = struct.unpack('<fff', buffer[1:-1])
                                self.signals.data_received.emit(f'{a}, {b}, {c}')
        ...
        if self.serial and self.serial.is_open:
            ...
            if text:
                try:
                    ...
                    codes = struct.pack('<fff', text[0], text[1], text[2])
                    check_sum = packet.compute_checksum(codes)

                    self.serial.write(b'\x55')
                    self.serial.write(codes)
                    self.serial.write(bytes([check_sum]))
